    ...
    for p in products:
        try:
            ...
            results.append(Product(
                ...
                category=category_name, store="Tommy Hilfiger",
                image_url=img,
            ))
        except Exception:
            continue
        ...
        with sync_playwright() as pw:
            ...
            for page_num in range(max_pages):
                ...
                try:
                    ...
                    if debug:
                        print(f"  [tommy] {category_name} p{page_num+1}: {len(api_data)} resp → {len(batch)} prods")

                    if not batch and page_num > 0:
                        break

                except PlaywrightTimeout:
                    logging.warning("[tommy] Timeout en %s", page_url)
                    break
                except Exception as e:
                    logging.error("[tommy] Error: %s", e)
                    break

            browser.close()

    except Exception as e:
        logging.error("[tommy] Error general: %s", e)
