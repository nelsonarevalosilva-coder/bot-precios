"""
Scraper para The Body Shop Chile — WooCommerce Store API.
Filtra directamente con ?on_sale=true, sin necesidad de calcular descuentos manualmente.
"""
import json
import time
...
API_BASE = "https://thebodyshop.cl/wp-json/wc/store/products"
PAGE_SIZE = 25
...
def scrape_category(
    ...
    while page <= max_pages:
        try:
            ...
            # BOM UTF-8 presente en la respuesta
            data = json.loads(resp.content.decode("utf-8-sig"))
        except Exception as e:
            if debug:
                print(f"  [thebodyshop] Error página {page}: {e}")
            break
        ...
        for item in data:
            ...
            if debug:
                print(f"  [thebodyshop] {name[:60]} — ${sale:,} (normal ${normal:,}) {discount_pct:.1f}%")
        ...
        if debug:
            print(f"  [thebodyshop] Página {page}/{total_pages}: {len(data)} items")
