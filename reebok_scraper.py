"""
Scraper para Reebok Chile (VTEX).
API: /api/catalog_system/pub/products/search?sl=outlet&_from=0&_to=49
...
def scrape_category(
    ...
    for page in range(max_pages):
        ...
        params = {"sl": "outlet", "_from": from_idx, "_to": to_idx}
        ...
        try:
            ...
            if debug:
                print(f"  [reebok] Error pagina {page}: {e}")
            break
        ...
        if debug:
            print(f"  [reebok] pagina {page}: {len(products)} productos, {found} con >={min_discount:.0f}%")
