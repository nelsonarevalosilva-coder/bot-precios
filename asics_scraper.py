"""
Scraper para ASICS Chile (VTEX).
API: /api/catalog_system/pub/products/search?sl=sale&_from=0&_to=49
...
def _sale_param(url: str) -> dict:
    """Extrae parámetro de búsqueda de la URL de categoría."""
    if "sl=" in url:
        sl = url.split("sl=")[-1].split("&")[0]
        return {"sl": sl}
    if "fq=" in url:
        fq = url.split("fq=")[-1].split("&")[0]
        return {"fq": fq}
    return {"sl": "sale"}
...
def scrape_category(
    ...
    for page in range(max_pages):
        ...
        params = {**params_base, "_from": from_idx, "_to": to_idx}
        ...
        try:
            ...
            if debug:
                print(f"  [asics] Error página {page}: {e}")
            break
        ...
        if debug:
            print(f"  [asics] página {page}: {len(products)} productos, {found} con >={min_discount:.0f}%")
