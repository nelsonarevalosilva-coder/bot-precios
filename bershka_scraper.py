    ...
    try:
        with Stealth().use_sync(sync_playwright()) as pw:
            ...
            try:
                ...
                if debug:
                    print(f"  [bershka] {category_name}: {page.title()[:60]} | respuestas: {len(api_responses)}")

            except PlaywrightTimeout:
                logging.warning("[bershka] Timeout en %s — usando lo capturado", category_name)
            except Exception as e:
                logging.error("[bershka] Error en %s: %s", category_name, e)

            browser.close()

    except Exception as e:
        logging.error("[bershka] Error general: %s", e)

    for data in api_responses:
        found = _parse_response(data, category_name, min_discount, seen)
        if found and debug:
            print(f"  [bershka] {len(found)} productos desde respuesta interceptada")
        all_products.extend(found)

    if debug:
        print(f"  [bershka] Total: {len(all_products)} productos >= {min_discount:.0f}%")
