    ...
    def handle_response(resp):
        try:
            if (
                ...
                and "stradivarius.com" in resp.url
            ):
                body = resp.json()
    ...
    try:
        with Stealth().use_sync(sync_playwright()) as pw:
            ...
            try:
                ...
                if debug:
                    print(f"  [stradivarius] {category_name}: {page.title()[:60]} | respuestas: {len(api_responses)}")

            except PlaywrightTimeout:
                logging.warning("[stradivarius] Timeout en %s — usando lo capturado", category_name)
            except Exception as e:
                logging.error("[stradivarius] Error en %s: %s", category_name, e)

            browser.close()

    except Exception as e:
        logging.error("[stradivarius] Error general: %s", e)

    for data in api_responses:
        found = _parse_response(data, category_name, min_discount, seen)
        if found and debug:
            print(f"  [stradivarius] {len(found)} productos desde respuesta interceptada")
        all_products.extend(found)

    if debug:
        print(f"  [stradivarius] Total: {len(all_products)} productos >= {min_discount:.0f}%")
