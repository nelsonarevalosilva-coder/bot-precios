"""
Scraper para Fila Chile (Magento 2 HTML).
Página de sale: /sale?p=N&product_list_limit=48
Precios en .old-price / .special-price, fallback por regex en .price-box.
"""
import re
import time
...
def _parse_product(item, category_name: str, min_discount: float) -> Product | None:
    try:
        name_el = item.select_one(".product-item-name a, .product-name a, a.product-name")
        if not name_el:
            name_el = item.select_one("a strong, .product-item-link")
        ...
        img = item.select_one("img.product-image-photo, img[src*='catalog/product']")
        ...
        if not normal_price or not sale_price:
            # Fallback: buscar precios por class nombres alternativos
            ...
            if len(all_prices) >= 2:
                ...
                return None  # sin descuento visible
...
def scrape_category(
    ...
    for page in range(1, max_pages + 1):
        page_url = f"{url}?p={page}&product_list_limit={PAGE_SIZE}" if page > 1 else f"{url}?product_list_limit={PAGE_SIZE}"
        try:
            resp = requests.get(page_url, headers=HEADERS, timeout=15)
            resp.raise_for_status()
        except Exception as e:
            if debug:
                print(f"  [fila] Error página {page}: {e}")
            break
        ...
        if debug:
            print(f"  [fila] página {page}: {len(items)} items, {found} con >={min_discount:.0f}%")
